...
# Проверка и исправление окончаний строк (CRLF -> LF)
if grep -q $'\r' "$0" 2>/dev/null || file "$0" | grep -q "CRLF"; then
    echo "Обнаружены Windows-окончания строк или смешанные окончания, исправляем..."
    # Сохраняем оригинальный файл
    cp "$0" "${0}.bak" || { echo "Ошибка: Не удалось создать резервную копию скрипта."; exit 1; }
    # Пытаемся исправить через sed
    if sed -i 's/\r$//' "$0" 2>/dev/null; then
        # Проверяем, остались ли CRLF
        if file "$0" | grep -q "CRLF"; then
            echo "Ошибка: sed не смог полностью исправить окончания строк."
            mv "${0}.bak" "$0" || { echo "Ошибка: Не удалось восстановить оригинальный файл."; exit 1; }
            # Устанавливаем dos2unix
            echo "Установка dos2unix..."
            sudo apt update && sudo apt install -y dos2unix || { echo "Ошибка: Не удалось установить dos2unix."; exit 1; }
            # Исправляем через dos2unix
            if dos2unix "$0" 2>/dev/null; then
                echo "Исправлено через dos2unix, перезапуск..."
                exec bash "$0" "$@"
            else
                echo "Ошибка: Не удалось исправить окончания строк через dos2unix."
                mv "${0}.bak" "$0" || { echo "Ошибка: Не удалось восстановить оригинальный файл."; exit 1; }
                exit 1
            fi
        else
            echo "Исправлено через sed, перезапуск..."
            exec bash "$0" "$@"
        fi
    else
        echo "Ошибка: Не удалось исправить окончания строк через sed."
        mv "${0}.bak" "$0" || { echo "Ошибка: Не удалось восстановить оригинальный файл."; exit 1; }
        # Устанавливаем dos2unix
        echo "Установка dos2unix..."
        sudo apt update && sudo apt install -y dos2unix || { echo "Ошибка: Не удалось установить dos2unix."; exit 1; }
